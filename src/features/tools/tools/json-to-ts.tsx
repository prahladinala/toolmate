...
export function JsonToTsTool({ tool }: { tool: ToolDef }) {
  ...
  const run = useCallback(() => {
    ...
    try {
      ...
      const tsInterfaces = JsonToTS(parsed).join("\n\n");
      setOutput(tsInterfaces);
      setError(null);
    } catch (err: any) {
      setError(err.message || "Failed to generate TypeScript interfaces.");
    }
  }, [input.value]);
    ...
    <main className="mx-auto max-w-6xl px-4 py-8">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <div className="rounded-[var(--radius-lg)] border border-[rgb(var(--border))] bg-[rgb(var(--card))] p-4">
          <div className="font-medium">Input JSON</div>
          <CodeEditor
        ...
        <div className="rounded-[var(--radius-lg)] border border-[rgb(var(--border))] bg-[rgb(var(--card))] p-4">
          <div className="flex items-center justify-between">
            <div className="font-medium">Output TypeScript Interfaces</div>
