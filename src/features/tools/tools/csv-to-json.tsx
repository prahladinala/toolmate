...
export function CsvToJsonTool({ tool }: { tool: ToolDef }) {
  ...
  const run = useCallback(() => {
    ...
    try {
      const parsed = Papa.parse(input.value, { header: true, skipEmptyLines: true });
    ...
    <main className="mx-auto max-w-6xl px-4 py-8">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <div className="rounded-[var(--radius-lg)] border border-[rgb(var(--border))] bg-[rgb(var(--card))] p-4">
          <div className="font-medium">Input CSV</div>
          <CodeEditor
        ...
        <div className="rounded-[var(--radius-lg)] border border-[rgb(var(--border))] bg-[rgb(var(--card))] p-4">
          <div className="flex items-center justify-between">
            <div className="font-medium">Output JSON</div>
