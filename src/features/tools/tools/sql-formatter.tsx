...
export function SqlFormatterTool({ tool }: { tool: ToolDef }) {
  ...
  const run = useCallback(() => {
    ...
    try {
      const formatted = format(input.value, { language: "sql", keywordCase: "upper" });
      setOutput(formatted);
      setError(null);
    } catch (err: any) {
    ...
    <main className="mx-auto max-w-6xl px-4 py-8">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <div className="rounded-[var(--radius-lg)] border border-[rgb(var(--border))] bg-[rgb(var(--card))] p-4">
          <div className="font-medium">Input SQL</div>
          <CodeEditor
        ...
        <div className="rounded-[var(--radius-lg)] border border-[rgb(var(--border))] bg-[rgb(var(--card))] p-4">
          <div className="flex items-center justify-between">
            <div className="font-medium">Formatted SQL</div>
